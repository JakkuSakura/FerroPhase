...
fn emit_file(file: &File, e: &mut KotlinEmitter) -> Result<()> {
    ...
    if !emitted_imports.is_empty() { e.writer.write_line(""); }
...
fn emit_item(
    ...
    match item.kind() {
        ...
        ItemKind::DefConst(c) => {
            ...
            e.writer.write_line(&format!("val {} = {}", name, val));
            Ok(())
        }
...
fn emit_trait(t: &fp_core::ast::ItemDefTrait, e: &mut KotlinEmitter) -> Result<()> {
    ...
    e.writer.write_line(&format!("interface {} {{", name));
    ...
    for item in &t.items {
        match item.kind() {
            ItemKind::DeclFunction(f) => {
                ...
                e.writer.write_line(&format!("fun {}({}){}", f.name.name.as_str(), params, ret));
            }
            ItemKind::DefFunction(f) => {
...
fn emit_struct(
    ...
    e.writer.write_line(&format!("data class {}(", name));
    for (i, field) in fields.iter().enumerate() {
        ...
        e.writer.write_line(&format!("    {} {}: {}{}", mutability, field.name.name, kt, comma));
    }
    ...
    e.writer.write_line(&format!("){} {{", header_suffix));
    e.writer.increase_indent();
...
fn emit_enum(
    ...
    if has_data {
        ...
        for (i, variant) in variants.iter().enumerate() {
            ...
            match &variant.value {
                ...
                Ty::Expr(expr) => {
                    ...
                    e.writer.write_line(&format!("    data class {}(val __data: {}) : {}()",
                        vname, ty_str, name));
                }
                _ => {
                    e.writer.write_line(&format!("    data class {}(vararg __data: Any?) : {}()", vname, name));
                }
            }
            if i < variants.len() - 1 { e.writer.write_line(""); }
        }
        if static_methods.is_empty() && traits.is_empty() {
            e.writer.write_line("}\n");
        } else {
        ...
        e.writer.write_line(&format!("enum class {}{} {{", name, header_suffix));
        for (i, variant) in variants.iter().enumerate() {
            let comma = if i < variants.len() - 1 || !static_methods.is_empty() || !traits.is_empty() { "," } else { "" };
            e.writer.write_line(&format!("    {}{}", variant.name.name.to_uppercase(), comma));
        }
        if static_methods.is_empty() && traits.is_empty() {
            e.writer.write_line("}\n");
        } else {
...
fn emit_companion_block(
    ...
    e.writer.write_line("companion object {");
    e.writer.increase_indent();
...
fn emit_companion_function(f: &ItemDefFunction, self_name: &str, e: &mut KotlinEmitter) -> Result<()> {
    ...
    e.writer.write_line(&format!("fun {}({}){} {{", name, params, ret));
    e.writer.increase_indent();
...
fn emit_impl_function(f: &ItemDefFunction, self_name: &str, e: &mut KotlinEmitter) -> Result<()> {
    ...
    e.writer.write_line(&format!("fun {}.{}({}){} {{", self_name, name, params, ret));
    e.writer.increase_indent();
    match untranspilable_reason(f) {
        Some(reason) => emit_stub_body(&format!("{self_name}::{name}"), reason, e),
        None => {
...
fn emit_override_function(f: &ItemDefFunction, self_name: &str, e: &mut KotlinEmitter) -> Result<()> {
    ...
    e.writer.write_line(&format!("override fun {}({}){} {{", name, params, ret));
    e.writer.increase_indent();
    match untranspilable_reason(f) {
        Some(reason) => emit_stub_body(&format!("{self_name}::{name}"), reason, e),
        None => {
                                ...
                                if let ExprKind::Name(name) = sel.obj.kind() {
                                    ...
                                    if e.split_iter_vars.contains(&obj_name) {
                                        ...
                                        if let Some(ref ty) = type_ann {
                                            e.writer.write_line(&format!("{} {} : {} = {}", decl_kw, var_name, ty, val));
                                        } else {
                                            e.writer.write_line(&format!("{} {} = {}", decl_kw, var_name, val));
                                        }
                                        return Ok(());
                                    }
                                }
    ...
    match expr.kind() {
        ...
        ExprKind::Return(ret) => {
            if let Some(val) = &ret.value {
                ...
                e.writer.write_line(&format!("return {}", v));
        ...
        _ => {
            ...
            if is_tail { e.writer.write_line(&format!("return {}", rendered)); }
            else { e.writer.write_line(&rendered); }
        }
    }
    Ok(())
}
    ...
    match expr.kind() {
        ...
        ExprKind::Match(mt) => {
            ...
            if is_single_arm || is_two_arm {
                ...
                buf.push_str(&"    ".repeat(e.writer.indent_depth()));
                buf.push('}');
                Ok(buf)
            }
        }
        ...
        ExprKind::Block(block) => {
            ...
            for stmt in &block.stmts {
                ...
                let saved = e.writer.swap_buffer(String::new());
                for _ in 0..1 + nest_depth { e.writer.increase_indent(); }
                emit_stmt(stmt, e, false)?;
                for _ in 0..1 + nest_depth { e.writer.decrease_indent(); }
                let stmt_text = e.writer.swap_buffer(saved);

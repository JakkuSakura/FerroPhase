...
pub struct ComptimeRequest {
    /// This request's own package. The checker passes its stable HIR view to
    /// the resolver separately, so the request only carries semantic identity.
...
/// interpretation) — supplied by `fp-compiler` when constructing a
/// `HirTypeChecker`, since only the driver knows how to do that; `fp-typing`
/// only knows *when* a request is needed
...
pub type ComptimeResolver = std::rc::Rc<
    dyn Fn(
        std::rc::Rc<fp_core::hir::HirProgram>,
        ComptimeRequest,
    ) -> BoxFuture<'static, fp_core::Result<Value>>,
>;

/// Tags a diagnostic as a hard item-check abort (see `record_item_check_failure`)
/// within `HirPackage::diagnostics`'s single unified manager.

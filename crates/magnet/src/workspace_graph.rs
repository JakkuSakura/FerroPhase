...
/// JSON-serializable snapshot of a `PackageGraph`, written by
/// `write_workspace_graph` — a pure serialization mirror of this crate's own
/// `PackageGraph`/`PackageNode`/`DependencyEdge` model, not an AST type.

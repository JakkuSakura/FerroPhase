...
fn test_magnet_cli_commands() -> Result<()> {
    ...
    assert!(
        ...
        "Magnet.toml missing [workspace] section"
    );
    assert!(
...
"#,
    ...
    assert!(
        output_str.contains("crate1"),
        "Tree output should include crate1"
    );
    assert!(
        output_str.contains("crate2"),
    ...
    assert!(
        content.contains("[workspace]"),
        "Cargo.toml missing [workspace] section"
    );
    assert!(
        content.contains("members"),
        "Cargo.toml missing workspace members"
    );

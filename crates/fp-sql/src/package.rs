    ...
    fn snapshot(&self) -> ProviderResult<AstPackage> {
        if !self.root.is_dir() {
            return Err(ProviderError::other(format!(
                "SQL project root is not a directory: {}",
                self.root.display()
            )));
        }
        ...
        if files.is_empty() {
            return Err(ProviderError::other(format!(
                "SQL project contains no `.sql` files: {}",
                self.root.display()
            )));
        }
        let frontend = crate::SqlFrontend::new();
        ...
        for file in files {
            ...
            let parsed = frontend.parse(&source, Some(&file)).map_err(|error| {
                ProviderError::other(format!(
                    "failed to parse SQL source {}: {error}",
                    file.display()
                ))
            })?;
            if module_paths.insert(module_path.clone()) {
                descriptors.push(ModuleDescriptor {
                    ...
                    language: ModuleLanguage::Other("sql".to_string()),
...
fn module_path_for(path: &Path) -> Vec<String> {
    ...
    if let Some(last) = parts.last_mut() {
        *last = last.trim_end_matches(".sql").to_string();
    }
    parts
}

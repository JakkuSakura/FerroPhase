    ...
    pub(super) fn lower_type_expr_with_context_and_substs(
        ...
        if let Some(ctx) = method_context {
            if let hir::TypeExprKind::Path(path) = &ty_expr.kind {
                if path.segments().first().map(|seg| seg.ident.as_str()) == Some("Self") {
                    ...
                    if path.segments().len() > 1 {
                        if let Some(assoc_name) = path.segments().get(1) {
                            if let Some(assoc_ty) = ctx.assoc_types.get(assoc_name.ident.as_str()) {
                    ...
                    if path.segments().len() > 1 {
                        if let Some(assoc_name) = path.segments().get(1) {
                            if let Some(assoc_ty) =
                                ctx.assoc_types.get(assoc_name.ident.as_str()).cloned()
                            {
                                return self
        ...
        match (&lhs.kind, &rhs.kind) {
            ...
            (hir::TypeExprKind::Path(a), hir::TypeExprKind::Path(b)) => {
                ...
                for (a_seg, b_seg) in a.segments().iter().zip(b.segments().iter()) {
                    ...
                    match (a_seg.explicit_args(), b_seg.explicit_args()) {
                        (None, None) => {}
                            ...
                            for (a_arg, b_arg) in a_args.args.iter().zip(b_args.args.iter()) {
                                match (a_arg, b_arg) {
                                    (hir::GenericArg::Type(a_ty), hir::GenericArg::Type(b_ty)) => {
                                        if !self.type_exprs_equivalent(a_ty.as_ref(), b_ty.as_ref()) {
                                            return false;
                                        }
                                    }
            ...
            if path.segments().len() == 1 && self.hir_item(def_id.clone()).is_none() {
                return Ty {
                    kind: TyKind::Param(mir::ty::ParamTy {
                        index: def_id.index,
                        name: path.segments()[0].ident.clone().into(),
                    }),
                };
            }

...
impl<'a> BodyBuilder<'a> {
    ...
    pub(super) fn resolve_self_qpath(&self, path: &mut hir::QPath) {
        if let hir::QPath::Resolved(_, ordinary_path) = path {
            self.resolve_self_path(ordinary_path);
        }
    }
        ...
        if let hir::ExprKind::Path(path) = &callee.kind {
            ...
            if tail == Some("get_unchecked") || tail == Some("::get_unchecked") {
                ...
                if let hir::ExprKind::Path(path) = &arg_values[0].kind {
                    ...
                    self.resolve_self_qpath(&mut resolved_path);
                ...
                if needs_result_ctor_infer {
                    ...
                    if inferred_args.is_none() {
                        if let hir::TypeExprKind::Path(path) = &self.function.sig.output.kind {
                            if path.path().is_some_and(|path| self.lowering.is_result_path(path)) {
                                if let Some(args) =
    ...
    pub(super) fn lower_operand(
        ...
        match &expr.kind {
            ...
            hir::ExprKind::Path(path) => {
                ...
                if !resolved_path.res_ref().is_some() {

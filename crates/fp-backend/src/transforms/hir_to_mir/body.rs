...
impl<'a> BodyBuilder<'a> {
    ...
    pub(super) fn resolve_self_path(&self, path: &mut hir::QPath) {
        if let Some(context) = &self.method_context {
            if let hir::QPath::Resolved(_, resolved) = path {
                if let Some(first) = resolved.segments().first() {
                    if first.ident.as_str() == "Self" {
                        let mut new_segments = context.path.clone();
                        new_segments.extend(resolved.segments().iter().skip(1).cloned());
                        resolved.segments = new_segments;
                        if let Some(ref def_id) = context.def_id {
                            resolved.res = hir::Res::Def(def_id.clone());
                        }
                    }
                }
            }
    ...
    pub(super) fn param_names_for_callee(&mut self, path: &hir::QPath) -> Option<Vec<hir::Symbol>> {
        match path.res_ref() {
    ...
    pub(super) fn callee_abi_from_path(&self, path: &hir::QPath) -> Option<(hir::Abi, bool)> {
        if let hir::Res::Def(def_id) = path.res_ref() {
    ...
    pub(super) fn lower_operand(
        ...
        match &expr.kind {
            ...
            hir::ExprKind::Path(path) => {
                ...
                if matches!(resolved_path.res_ref(), hir::Res::Error) {
                    if let Some(variant) = self.enum_variant_info_from_qpath(&resolved_path) {
                        let mut layout = expected.and_then(|ty| {
                            self.enum_layout_for_variant(&variant, Some(ty), expr.span)
                        });
                        if layout.is_none() {
                ...
                if has_explicit_args {
                    let method_def = self
                        .lowering
                        .typeck_method_resolution(expr.hir_id.clone())
                        .or_else(|| match resolved_path.res_ref() {
                            hir::Res::Def(def_id) => Some(def_id.clone()),
                            _ => None,
                        })

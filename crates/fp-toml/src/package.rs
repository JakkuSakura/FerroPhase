...
#[derive(Debug)]
pub struct TomlPackageProvider {
    #[allow(dead_code)]
    root: PathBuf,
}

impl TomlPackageProvider {
    ...
    fn snapshot(&self) -> ProviderResult<AstPackage> {
        if !self.root.is_dir() {
            return Err(ProviderError::other(format!(
                "TOML project root is not a directory: {}",
                self.root.display()
            )));
        }
        ...
        if files.is_empty() {
            return Err(ProviderError::other(format!(
                "TOML project contains no `.toml` files: {}",
                self.root.display()
            )));
        }
        ...
        for file in files {
            ...
            let source = std::fs::read_to_string(&file).map_err(|error| {
                ProviderError::other(format!(
                    "failed to read TOML source {}: {error}",
                    file.display()
                ))
            })?;
            let parsed = frontend.parse(&source, Some(&file)).map_err(|error| {
                ProviderError::other(format!(
                    "failed to parse TOML source {}: {error}",
                    file.display()
                ))
            })?;
            if module_paths.insert(module_path.clone()) {
                descriptors.push(ModuleDescriptor {
                    ...
                    language: ModuleLanguage::Other("toml".to_string()),
...
impl PackageProvider for TomlPackageProvider {
...
fn collect_files(root: &Path, files: &mut Vec<PathBuf>) -> ProviderResult<()> {
    for entry in std::fs::read_dir(root).map_err(|error| {
        ProviderError::other(format!(
            "failed to read TOML directory {}: {error}",
            root.display()
        ))
    })? {
        let path = entry
...
fn module_path_for(path: &Path) -> Vec<String> {
    ...
    if let Some(last) = parts.last_mut() {
        *last = last.trim_end_matches(".toml").to_string();
    }
    parts
}

generated: crates/fp-shell/examples/runtime-variables.sh
=0
...
restart_service() {
    local host="$1"
    local service="$2"
    shell_run 'localhost' "echo restarting service=${service} on host=${host}" '' '' '' ''
    shell_run "${host}" "sudo systemctl restart ${service}" '' '' '' ''
}

local service='fp-service'
for host in 'web-1' 'web-2'; do
    shell_run 'localhost' "echo loop host=${host}" '' '' '' ''
    shell_run "${host}" "hostname" '' '' '' ''

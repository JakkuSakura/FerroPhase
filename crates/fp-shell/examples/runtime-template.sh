generated: crates/fp-shell/examples/runtime-template.sh
d=0
...
fp_validate_runtime() {
  command -v 'cp' >/dev/null 2>&1 || { echo "missing required command: cp" >&2; exit 1; }
...
shell_template 'localhost' './templates/fp-service.conf.tpl' '/etc/fp-service/fp-service.conf' '' '' '' '' ''

...
common_enum! {
    ...
    pub enum ItemKind {
        ...
        /// straight off `AstPackage.items`, the
        ...
        /// `emit_package`, exactly the same shape `NativeEmitter` already has for
...
impl Item {
    ...
    pub fn get_ident(&self) -> Option<&Ident> {
        match self.kind() {
            ...
            ItemKind::DefStatic(define) => Some(&define.name),
            ItemKind::DefTrait(define) => Some(&define.name),
            ItemKind::DeclType(declare) => Some(&declare.name),
            ItemKind::DeclConst(declare) => Some(&declare.name),
            ItemKind::DeclStatic(declare) => Some(&declare.name),
